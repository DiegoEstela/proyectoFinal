...
export class ProductoMongo {
  ...
  async save(producto) {
    try {
      const productoGuardado = await this.modelo.create({
        ...producto,
  ...
  async updateById(id, producto) {
    try {
      const productoActualizado = await this.modelo.findByIdAndUpdate(
        id,
        producto
      );
...
export class CarritoMongo {
  ...
  async saveCart() {
    try {
      const cartDB = await this.modelo.create({
        timestamp: new Date().toLocaleString(),
        checkout: false,
        productos: [],
      });
  ...
  async updateCart(nuevoElem) {
    try {
      const data = await this.modelo.replaceOne(
        { _id: nuevoElem._id },
        nuevoElem
      );
      return data;
